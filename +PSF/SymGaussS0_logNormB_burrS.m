classdef SymGaussS0_logNormB_burrS < PSF.SymGaussS0
    % a symmetric Gaussian PSF model with width offset (SymGaussS0), with
    % log-normal background amplitude priors, i.e., a normal prior on the
    % fit parameter lnB, with mean value parameters lnB0 and std-parameter
    % lnBstd, and Burr distribution (see documentation of the matlab class
    ...
    %
    % Construction:
    % P = SymGaussS0_logNormB_burrS('initialGuess',[mux muy lnB lnN lnS],'S0','priorParameters',[lnB0 lnBstd alpha c k]),
    % P = SymGaussS0_logNormB_burrS('initialGuess',[mux muy lnB lnN lnS],'lambda',lambda,'NA',NA,'priorParameters',[lnB0 lnBstd alpha c k]),
    ...
    methods
        % Constructor
        function this = SymGaussS0_logNormB_burrS(varargin)
            % call superclass constructor
            this@PSF.SymGaussS0(varargin{:});
            % sanity check on priors
            if(numel(this.priorParameters)~=5 )
               error('PSF.SymGaussS0_logNormB_burrS needs 5 prior parameters [lnB0 lnBstd alpha c k]')
            end
