...
function [lnZ,wA,pst]=HMM_multiForwardBackward_startend_m(Q,H,iStart,iEnd,doBackward) 
...
for n=1:length(iEnd)
    ...
    if(doBackward) % backward sweep
        ...
        % occuptional probabilities
        pst(tStart:tEnd,:)=rowNormalize(alpha(tStart:tEnd,:).*beta(tStart:tEnd,:));
    end    
end
